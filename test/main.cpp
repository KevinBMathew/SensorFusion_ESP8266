...
const char* ap_ssid = "ESP8266-TempMonitor";
...
<head>
  ...
  <style>
    ...
    .sensor-name { font-size: 1.2rem; font-weight: bold; color: #34495e; }
    .temp-value { font-size: 1.8rem; color: #e74c3c; }
    .extremes { font-size: 1rem; color: #7f8c8d; margin-top: 5px; }
    .status { font-size: 0.9rem; color: #95a5a6; }
  </style>
</head>
<body>
  <h2>ESP8266 Temperature Monitor</h2>
  <div id="sensors"></div>
  <div class="status">
...
SensorPort sensors[] = {
  ...
  {7, &readSHT45, "7.SHT45", -INFINITY, INFINITY, 40, 0}
};

float getCurrentAverage(int sensorIndex) {
  if (readingIndex[sensorIndex] == 0) return NAN;
  
  float sum = 0;
  int count = readingIndex[sensorIndex];
  
  for (int i = 0; i < count; i++) {
    sum += sensorReadings[sensorIndex][i];
  }
...
String generateSensorHTML() {
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    // Get current average if available
    float currentTemp = getCurrentAverage(i);
    if (!isnan(currentTemp)) {
      html += "<div class='temp-value'>" + String(currentTemp, 2) + "°C</div>";
    } else {
      html += "<div class='temp-value'>--°C</div>";
    }
    
    // Show extremes
    html += "<div class='extremes'>";
    if (sensors[i].maxTemp != -INFINITY && sensors[i].minTemp != INFINITY) {
      html += "Max: " + String(sensors[i].maxTemp, 2) + "°C | ";
      html += "Min: " + String(sensors[i].minTemp, 2) + "°C";
    } else {
      html += "Max: -- | Min: --";
    }
    html += "</div>";
    ...
    html += "<div class='extremes'>";
    html += "Readings: " + String(readingIndex[i]) + "/12";
    html += "</div>";
...
void setup() {
  ...
  server.on("/sensors", HTTP_GET, [](AsyncWebServerRequest *request){
    String html = generateSensorHTML();
